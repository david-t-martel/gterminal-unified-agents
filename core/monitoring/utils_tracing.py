...
try:
    ...
    # Fallback when Google Cloud libraries are not available
...
if TRACING_AVAILABLE:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.bucket_name = bucket_name or f"{self.project_id}-my-fullstack-agent-logs-data"
    ...
    def store_in_gcs(self, content: str, span_id: str) -> str:
        ...
        if not self.storage_client.bucket(self.bucket_name).exists():
            logging.warning(
                f"Bucket {self.bucket_name} not found. Unable to store span attributes in GCS."
            )
    ...
    def _process_large_attributes(self, span_dict: dict, span_id: str) -> dict:
        ...
        if len(json.dumps(attributes).encode()) > 255 * 1024:  # 250 KB
            ...
            attributes_retain["url_payload"] = (
                f"https://storage.mtls.cloud.google.com/{self.bucket_name}/spans/{span_id}.json"
            )

            span_dict["attributes"] = attributes_retain
            logging.info(
                "Length of payload span above 250 KB, storing attributes in GCS to avoid large log entry errors",
            )

        return span_dict

else:
    # Fallback class when tracing is not available
    class CloudTraceLoggingSpanExporter:
        """Fallback exporter when Google Cloud libraries are not available."""

        def __init__(self, *args, **kwargs) -> None:
            pass

        def export(self, spans) -> None:
            """No-op export when tracing is not available."""
            return
