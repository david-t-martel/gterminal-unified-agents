...
class CodeAnalysisTool(Tool):
    ...
    def _analyze_complexity(self, content: str, language: str) -> int:
        ...
        control_keywords = {
            ...
            "typescript": ["if", "else", "for", "while", "switch", "case", "try", "catch", "finally"],
            "rust": ["if", "else", "for", "while", "loop", "match", "if let", "while let"],
            "go": ["if", "else", "for", "switch", "case", "select"],
            "java": ["if", "else", "for", "while", "switch", "case", "try", "catch", "finally"]
        }
    ...
    def _generate_directory_summary(
        ...
        dominant_lang = max(language_stats.items(), key=lambda x: x[1]["lines"])[0] if language_stats else "unknown"
